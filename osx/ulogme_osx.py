...
class AppDelegate(NSObject):
    ...
    def writeActiveApp_(self, timer):
        if DEBUG_APP:
            print(f"Got active app callback at {current_time()}")
...
class EventSniffer:
    ...
    def start_test_timer(self):
        NSTimer.scheduledTimerWithTimeInterval_target_selector_userInfo_repeats_(
            1, self.delegate, "timerCallback:", None, True
        )
    ...
    def run(self):
        ...
        nc.addObserver_selector_name_object_(
            self.delegate, "applicationActivated:", "NSWorkspaceDidActivateApplicationNotification", None
        )

        nc.addObserver_selector_name_object_(
            self.delegate, "screenSleep:", "NSWorkspaceScreensDidSleepNotification", None
        )
    ...
    def event_handler(self, event):
        if event.type() == NSKeyDown:
            if DEBUG_KEYSTROKE:
                print(f"Got keystroke in {self.current_app}")
...
if __name__ == "__main__":
    option_list = [
        make_option("--pid_file", action="store", dest="pid_file", default='.python_pid', help="Required."),
        make_option("--keystroke_raw_file", action="store", dest="keystroke_raw_file", default='keystroke.txt', help="Required."),
        make_option("--active_window_file", action="store", dest="active_window_file", default='window.txt', help="Required."),
        make_option(
            "--active_window_time",
            action="store",
